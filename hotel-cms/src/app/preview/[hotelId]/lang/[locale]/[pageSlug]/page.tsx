...
interface LocalePreviewPageProps {
  params: Promise<{ hotelId: string; locale: string; pageSlug: string }>;
}

export default async function LocalePreviewPage({ params }: LocalePreviewPageProps) {
  const { hotelId, locale, pageSlug } = await params;
  ...
  // Fetch the page by slug + hotelId + locale; fallback to defaultLocale
  let page = await prisma.page.findFirst({
    where: { hotelId, slug: resolvedSlug, locale },
    include: {
      sections: {
  ...
  if (!page) {
    // Fallback: try with hotel's defaultLocale
    page = await prisma.page.findFirst({
      where: { hotelId, slug: resolvedSlug, locale: hotel.defaultLocale ?? undefined },
      include: {
        sections: {
          orderBy: { sortOrder: "asc" },
        },
  ...
  const pageTitle = metaTags?.title ?? `${hotel.name} [${locale}] — Preview`;
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-amber-500 text-stone-900 text-xs text-center py-2 font-medium tracking-wide">
        Preview Mode — {hotel.name} · {page.slug} · {locale.toUpperCase()} ·{" "}
...
export async function generateMetadata({ params }: LocalePreviewPageProps) {
  const { hotelId, locale, pageSlug } = await params;
  const resolvedSlug = pageSlug === "home" ? "/" : pageSlug;
  ...
  return {
    title: metaTags?.title ?? `${page.hotel.name} [${locale}] — ${page.slug}`,
